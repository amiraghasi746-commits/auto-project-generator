...
MY_GITHUB_PAT      = os.environ["MY_GITHUB_PAT"]          # ← Personal Access Token
MY_GITHUB_USERNAME = os.environ["MY_GITHUB_USERNAME"]
...
def create_github_repo(project: dict, files: dict[str, str]) -> str:
    g = Github(MY_GITHUB_PAT)          # FIX ۲: از PAT استفاده می‌کنیم
    user = g.get_user()
